...
lexicographic recursive permutation algorithm.
...
The elements of the array must be distinct and
comparable to determine their order.
...
# The variable all_integers is used later in:
# Tab.sort(key=int, reverse=True)
# to obtain numerical order rather than simple lexicographic
# order of strings.
all_integers = True
...
# ================================================================
# Sort the elements in decreasing order because the
# Woodall algorithm displays the array from right to left.
try:
    if all_integers:
        # For integers, key=int ensures that the elements
...
LEXPERM(Tab, number_of_elements)